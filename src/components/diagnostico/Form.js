...
import { save, getById, update } from '../../actions/diagnostico-action'
    ...
    handleSubmit = event => {
        ...
        if (id) {
            this.props.update(this.state, this.props.history).then(r => {
                r.push('/catalogo/diagnosticos/list')
            }, error => {
                throw (error)
            })
        } else {
...
const mapStateToProps = (state, props) => {
    if (props.match.params.id) {
        return {
            data: state.diagnostico.list.find(item => item.id + '' === props.match.params.id + '')
        }
    }
    return {
        data: null
